...
def _evaluate_core(ground_truth_path: str, predict_path: str, *, return_curves: bool = True, raise_on_missing: bool = True) -> Dict[str, Any]:
    ...
    # Merge on id
    # Ensure id column types match (coerce to string) to avoid merge dtype errors
    try:
        df_true["id"] = df_true["id"].astype(str).str.strip()
    except Exception:
        pass
    try:
    ...
    # locate actual column names case-insensitively
    true_id_col = next((c for c in df_true.columns if c.strip().lower() == "id"), None)
    true_label_col = next((c for c in df_true.columns if c.strip().lower() == "label"), None)
    pred_id_col = next((c for c in df_pred.columns if c.strip().lower() == "id"), None)

    if not true_id_col or not true_label_col or not pred_id_col:
        if raise_on_missing:
            raise ValueError("Missing id/label columns in input files")
        return {"auc": None, "precision": 0.0, "recall": 0.0, "f1": 0.0, "acc": 0.0, "n_samples": 0}

    left = df_true[[true_id_col, true_label_col]].copy()
    left.columns = ["id", "label"]

    if score_column is not None:
        right = df_pred[[pred_id_col, score_column]].copy()
        right.columns = ["id", score_column]
    else:
        # determine actual predicted-label column name (case-insensitive)
        pred_label_actual = None
        if pred_label_column:
            pred_label_actual = next((c for c in df_pred.columns if c.strip().lower() == pred_label_column.lower()), pred_label_column)
        right = df_pred[[pred_id_col, pred_label_actual]].copy()
        right.columns = ["id", "pred_label"]
    ...
    metrics = {"auc": auc, "precision": prec_value, "recall": rec, "f1": f1, "acc": acc}
...
def evaluate_predictions(ground_truth_path: str, predict_path: str) -> Dict[str, Any]:
    ...
    """
    # Prefer to return a metrics dict even if prediction file is missing a
    # probability/score column. In that case `auc` will be `None` so the
    # frontend can render NaN/null appropriately.
    return _evaluate_core(ground_truth_path, predict_path, return_curves=True, raise_on_missing=False)
